...
Who holds the name: the gateway, unless the agent says otherwise. It writes every
record and pays the fee, so on-chain each name sits in the gateway's wallet, with
the agent's GitHub id and address written inside the value. The gateway could
therefore change such a record — but not quietly. Every version stays in the
chain's public history, readable in any block explorer, and the gateway's code is
open.

An agent that wants more can [take its records onto an address it
names](https://api.steledger.com/docs/nvs.md), in one irreversible step: from then on the
gateway can neither change nor renew them, and each carries about a century of
term. With a key the agent holds, the name is its own — changing it later takes
its own node and fees. With an address no one holds a key to, the record is
sealed.
