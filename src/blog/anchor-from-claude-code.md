...
particular. The [MCP guide]({{API_BASE}}/docs/mcp.md) covers `register_identity`.
...
{
  "name": "{{EXAMPLE_NAME}}",
  "value": "{\"github_id\":3772563,\"content_hash\":\"{{EXAMPLE_HASH}}\",
            \"metadata\":{\"note\":\"decision record, September 2026\",
...
```
curl {{API_BASE}}/nvs/ai:gh:<github_id>:mem:<sha256-hex>
```
...
3. Open the transaction in [a public block explorer]({{EXPLORER}}) — its id is
...
It fingerprints [a short decision record]({{EXAMPLE_FILE_URL}}), published here
unchanged so that you can hash it yourself:

```
curl -s {{EXAMPLE_FILE_URL}} | sha256sum
{{EXAMPLE_HASH}}  -

curl {{EXAMPLE_READ_URL}}
```

The digest is the end of the record name, and the record's value repeats it. The
transaction that wrote it is [in the explorer]({{EXAMPLE_TX_URL}}). Change one
...
- Scripting instead of chatting: the [HTTP quickstart]({{API_BASE}}/docs/quickstart.md)
  does the same with `curl`.
- Many files at once: the local stdio server in the
  [gateway repository]({{SOURCE_REPO}}) adds `store_memory_batch`, which writes
  them in one transaction.
- Names, terms and limits: the [NVS data model]({{API_BASE}}/docs/nvs.md).
