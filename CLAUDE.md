...
- The live service (API, MCP server, OAuth, docs) runs at **ai.emercoin.com**.
  This site does NOT host the API and does NOT duplicate the docs — it links to them.
...
docs: https://ai.emercoin.com/llms-full.txt (and https://ai.emercoin.com/openapi.json).
If the site and those docs disagree, the docs win — fix the site.

Stable facts (re-verify before relying on them):
- MCP endpoint: `https://ai.emercoin.com/mcp` (Streamable HTTP).
...
- Claude Code: `claude mcp add --transport http steledger https://ai.emercoin.com/mcp`,
  then `/mcp` to sign in.
...
- Do not copy the ai.emercoin.com docs into this repo — link to them.
